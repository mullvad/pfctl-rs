...
impl PfCtl {
    ...
    pub fn flush_rules(&mut self, anchor: &str, kind: RulesetKind) -> Result<()> {
        let trans = Transaction::new(&anchor, kind)?;
        trans.commit()
    }
...
impl Transaction {
    ...
    pub fn new(anchor: &str, kind: RulesetKind) -> Result<Self> {
        let file = open_pf()?;
        let ticket = Transaction::get_ticket(file.as_raw_fd(), &anchor, kind)?;
        Ok(
            Transaction {
                file: file,
                ticket: ticket,
    ...
    pub fn add_rules(&self, rules: &[FilterRule]) -> Result<()> {
        let pool_ticket = get_pool_ticket(self.fd(), &self.anchor)?;
        for rule in rules.iter() {
            self.add_rule(pool_ticket, &rule)?;
        }
        Ok(())
    }
    ...
    /// Add single rule into transaction
    fn add_rule(&self, pool_ticket: u32, rule: &FilterRule) -> Result<()> {
        let mut pfioc_rule = unsafe { mem::zeroed::<ffi::pfvar::pfioc_rule>() };
    ...
    fn get_ticket(fd: RawFd, anchor: &str, kind: RulesetKind) -> Result<u32> {
        let mut pfioc_trans_e = unsafe { mem::zeroed::<ffi::pfvar::pfioc_trans_e>() };
        Transaction::setup_trans_element(&anchor, kind, &mut pfioc_trans_e)?;
...
impl TryCopyTo<ffi::pfvar::pfioc_trans_e> for Transaction {
    fn copy_to(&self, pfioc_trans_e: &mut ffi::pfvar::pfioc_trans_e) -> Result<()> {
        ...
        Transaction::setup_trans_element(&self.anchor, self.kind, pfioc_trans_e)
    }
}
