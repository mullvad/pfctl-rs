...
test!(add_filter_anchor {
    ...
    assert_matches!(pf.add_anchor(anchor_name.as_str(), pfctl::AnchorKind::Filter), Ok(()));
...
test!(remove_filter_anchor {
    ...
    assert_matches!(pf.add_anchor(anchor_name.as_str(), pfctl::AnchorKind::Filter), Ok(()));
    assert_matches!(pf.remove_anchor(anchor_name.as_str(), pfctl::AnchorKind::Filter), Ok(()));
