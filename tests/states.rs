...
fn send_udp_packet(sender: SocketAddr, recepient: SocketAddr) -> Result<usize, std::io::Error> {
    UdpSocket::bind(sender)?.send_to(&[0], recepient)
}
...
test!(reset_ipv4_states_by_anchor {
    ...
    let rule = rule_builder()
            .to(pfctl::Endpoint::from(server_addr))
            .build()
            .unwrap();
    pf.set_rules(ANCHOR_NAME, &[rule]).unwrap();

    send_udp_packet(sender_addr, server_addr).unwrap();
