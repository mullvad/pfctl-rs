...
fn get_test_filter_rules() -> Vec<pfctl::FilterRule> {
...
fn get_test_redirect_rules() -> Vec<pfctl::RedirectRule> {
...
fn get_test_marker_filter_rule() -> pfctl::FilterRule {
...
fn get_test_marker_redirect_rule() -> pfctl::RedirectRule {
...
fn assert_eq_test_filter_rules(pf_rules: Vec<String>) {
    assert_eq!(
        pf_rules,
        vec![
...
fn assert_eq_test_redirect_rules(pf_rules: Vec<String>) {
    assert_eq!(
        pf_rules,
        vec![
...
fn assert_eq_test_filter_marker_rule(pf_rules: Vec<String>) {
    assert_eq!(pf_rules, vec!["block drop all"]);
}

fn assert_eq_test_redirect_marker_rule(pf_rules: Vec<String>) {
    assert_eq!(
        pf_rules,
...
test!(replace_many_rulesets_in_one_anchor {
    ...
    change.set_filter_rules(get_test_filter_rules());
    change.set_redirect_rules(get_test_redirect_rules());
    assert_matches!(pf.set_rules(vec![change]), Ok(()));
    assert_eq_test_filter_rules(pfcli::get_rules(ANCHOR1_NAME).unwrap());
    assert_eq_test_redirect_rules(pfcli::get_nat_rules(ANCHOR1_NAME).unwrap());
});
...
test!(replace_one_ruleset_in_many_anchors {
    ...
    assert_matches!(pf.add_rule(ANCHOR1_NAME, &get_test_marker_filter_rule()), Ok(()));
    assert_eq_test_filter_marker_rule(pfcli::get_rules(ANCHOR1_NAME).unwrap());

    assert_matches!(pf.add_redirect_rule(ANCHOR2_NAME, &get_test_marker_redirect_rule()), Ok(()));
    assert_eq_test_redirect_marker_rule(pfcli::get_nat_rules(ANCHOR2_NAME).unwrap());

    let mut change1 = pfctl::AnchorChange::new(ANCHOR1_NAME);
    change1.set_redirect_rules(get_test_redirect_rules());

    let mut change2 = pfctl::AnchorChange::new(ANCHOR2_NAME);
    change2.set_filter_rules(get_test_filter_rules());
    ...
    assert_eq_test_filter_rules(pfcli::get_rules(ANCHOR2_NAME).unwrap());
    assert_eq_test_redirect_marker_rule(pfcli::get_nat_rules(ANCHOR2_NAME).unwrap());
});
