...
pub fn get_rules<S: AsRef<str>>(anchor_name: S) -> Result<Vec<String>> {
    let output = get_command()
        .arg("-a")
        .arg(OsStr::new(anchor_name.as_ref()))
        .arg("-sr")
        .output()
        .chain_err(|| "Failed to run pfctl")?;
    let output = str_from_stdout(&output.stdout)?;
...
pub fn flush_rules<S: AsRef<str>>(anchor_name: S, options: FlushOptions) -> Result<()> {
