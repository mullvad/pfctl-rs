...
test!(enable_pf {
    ...
    assert_matches!(pf.enable(), Err(pfctl::Error{ source: pfctl::ErrorSource::StateAlreadyActive(_), .. }));
...
test!(disable_pf {
    ...
    assert_matches!(pf.disable(), Err(pfctl::Error { source: pfctl::ErrorSource::StateAlreadyActive(_), .. }));
